...
class AnimateIt {
    ...
    set animationPosition(y) {
        this._startPosition = evaluatePosition(y);
    }
...
class Helper {
    static hideElement(target, anim) {
    ...
    Array.from(allDataAi, (element) => {
        ...
        window.addEventListener('scroll', () => {
            ...
            if (now - lastCall > 100) {
                lastCall = now;
                target.updatePosition = element;
                (Helper.isElementInsideOfScreen(target, element, anim)) ? anim.animateIt(element) : Helper.hideElement(target, anim);
            }
        });
    });
}
